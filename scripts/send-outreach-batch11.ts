...
async function main() {
  ...
  const rows = parseCSV(fs.readFileSync(
    process.env.HOME + "/Downloads/dataset_leads-finder_2026-04-27_13-59-42-501.csv", "utf-8"
  ));

  const leads = rows.filter(r => r.email && r.email.includes("@") && !alreadySent.has(r.email.toLowerCase()));
  console.log(`\n📤 Batch 11 — sending to ${leads.length} new leads\n`);
...
<p>Mercier<br/>Founder @ Dentago<br/><a href="https://www.dentago.co.uk">www.dentago.co.uk</a></p>`;
    ...
    try {
      ...
      sentData.sent.push({ name, email, practice: company, batch: 11, template: "Batch11", sentAt: new Date().toISOString() });
      sent++;
    } catch (e: any) {
