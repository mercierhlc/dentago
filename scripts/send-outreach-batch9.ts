...
async function main() {
  ...
  const alreadySent = new Set(sentData.sent.map((s: any) => s.email.toLowerCase()));

  const rows = parseCSV(fs.readFileSync(
    process.env.HOME + "/Downloads/dataset_leads-finder_2026-04-26_12-22-41-573.csv", "utf-8"
  ));

  const leads = rows.filter(r => r.email && r.email.includes("@") && !alreadySent.has(r.email.toLowerCase()));
...
<img src="https://www.dentago.co.uk/api/track/open?e=${encodeURIComponent(email)}&b=9" width="1" height="1" style="display:none" />`;
    ...
    try {
      ...
      sentData.sent.push({ name, email, practice: company, batch: 9, template: "Batch9-Apify", sentAt: new Date().toISOString() });
      sent++;
    } catch (e: any) {
  ...
  console.log(`\nBatch 9 done: ${sent} sent, ${failed} failed`);
