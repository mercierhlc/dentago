...
function renderContent(content: string) {
      ...
      <ul key={key++} className="my-6 space-y-3">
        {listBuffer.map((item, i) => (
          <li key={i} className="flex gap-3 text-lg text-slate-700 leading-relaxed">
            <span className="mt-1.5 w-2 h-2 rounded-full bg-[#111111] flex-shrink-0" />
  ...
  for (const line of lines) {
    ...
    if (trimmed.match(/^\[.+\]\(.+\)$/)) {
      ...
      if (match) {
        elements.push(
          <div key={key++} className="my-12 p-8 bg-gradient-to-br from-[#111111]/5 to-[#111111]/10 rounded-2xl border border-[#111111]/20 text-center">
            ...
            <Link
              href={match[2]}
              className="inline-flex items-center gap-2 bg-[#111111] text-white font-bold px-8 py-4 rounded-xl hover:bg-[#5b30d6] transition-colors text-base shadow-lg shadow-[#111111]/25"
            >
              {match[1]} →
            </Link>
          </div>
    ...
    if (isStatLine) {
      flushList();
      elements.push(
        <div key={key++} className="my-8 pl-6 border-l-4 border-[#111111] bg-[#111111]/5 rounded-r-xl py-4 pr-4">
          <p
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="relative w-full h-[400px] sm:h-[500px] overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8 sm:p-14 max-w-4xl mx-auto">
          <div className="flex items-center gap-3 mb-4">
            <span className="text-xs font-bold bg-[#111111] text-white px-3 py-1 rounded-full">{post.category}</span>
      ...
      <main className="max-w-3xl mx-auto px-6 py-14">
        ...
        <div className="flex items-center gap-2 text-sm text-slate-400 mb-10">
          <Link href="/blog" className="hover:text-[#111111] transition-colors font-medium">← Back to blog</Link>
        </div>

        {/* Description */}
        <p className="text-xl text-slate-500 leading-relaxed border-l-4 border-[#111111] pl-6 mb-12 font-medium">
          {post.description}
        </p>
        ...
        <div className="mt-16 pt-8 border-t border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-white ring-1 ring-slate-200 flex items-center justify-center shadow-sm">
              <DentagoLogo size={26} variant="brand" />
            </div>
            <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {otherPosts.map((p) => (
                <Link
                  key={p.slug}
                  href={`/blog/${p.slug}`}
                  className="group bg-white rounded-2xl overflow-hidden border border-slate-100 hover:border-[#111111]/30 hover:shadow-lg transition-all duration-200"
                >
                  ...
                  <div className="p-6">
                    <span className="text-xs font-bold bg-[#111111]/10 text-[#111111] px-3 py-1 rounded-full">{p.category}</span>
                    <h3 className="text-base font-extrabold text-slate-900 mt-3 mb-2 leading-snug group-hover:text-[#111111] transition-colors">{p.title}</h3>
                    <p className="text-sm text-slate-500 line-clamp-2">{p.description}</p>
                    <div className="mt-4 text-sm font-bold text-[#111111]">Read article →</div>
                  </div>
                </Link>
              ))}
            </div>
      ...
      <footer className="border-t border-slate-100 py-10">
        <div className="max-w-5xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <Link href="/signup" className="text-sm font-bold text-[#111111] hover:underline">Start for free →</Link>
        </div>
      </footer>
    </div>
  );
